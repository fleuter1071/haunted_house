...
### 2026-05-17 00:00 - Stranger Things Signal Puzzle And Audio Pass

- **Feature name, work name, description, and value provided:** Transformed the game further toward a Hawkins/Upside Down rescue theme, added intro music controls, replaced the study symbol puzzle with a Christmas lights plus radio-frequency puzzle, and added a Demogorgon room sound in the study. This made the core puzzle and audio feedback more thematically recognizable and more interactive.
- **Files changed:** `index.html`, `styles.css`, `script.js`, `assets/audio/upside-down-theme.mp3`, `assets/audio/demogorgon_sound.mp3`, multiple SVG assets from the Hawkins/Upside Down visual pass
- **Technical Architecture changes or key technical decisions made:** Added browser audio handling with best-effort autoplay, interaction retry, looping intro music, persisted intro music preference via `localStorage`, and a separate study-room Demogorgon loop that starts in the study and stops/resets when leaving. Replaced `libraryPuzzle` symbol-order state with radio state: `clueSeen`, `radioOpen`, `selectedDigit`, `frequency`, `answer`, `solved`, and `penaltyTimer`. Added Canvas rendering helpers for the Christmas light wall, radio prop, and radio tuning overlay.
- **Assumptions:** Browser autoplay may be blocked until the player interacts, so audio should try on load but retry after click/key input. The puzzle answer is `415`, revealed by the light wall as `FOUR ONE FIVE`. The Demogorgon sound should be scoped to the study because that is where the enemy pressure is most relevant.
- **Known limitations:** Audio controls currently govern intro music only, not the Demogorgon room sound. The radio puzzle is keyboard-only and does not yet support mouse/touch controls. The Stranger Things naming/theme is suitable for a local fan prototype, but would create IP concerns for public or commercial use. No automated visual or audio regression tests exist.
- **Key learnings that you can bring with you to future sessions:** Stronger theme comes from turning recognizable story beats into mechanics, not just renaming rooms. Browser audio needs user-interaction fallback logic. Room-scoped ambient sounds should pause and reset when leaving the room so audio state does not leak across gameplay.
- **Remaining TODOs:** Playtest the full flow from study entry through light reveal, radio tuning, key pickup, gate unlock, lab rescue, and audio transitions. Consider adding a global sound-effects toggle, mouse/touch support for the radio overlay, and clearer on-canvas prompts if players miss the light-wall clue.
- **Next steps:** Continue polishing the study puzzle feedback, then decide whether to keep extending the custom Canvas engine or prototype a Phaser branch if gameplay scope grows.

### 2026-05-22 00:00 - Upside Down Rift Payoff

- **Feature name, work name, description, and value provided:** Added a cinematic Upside Down rift payoff after the player correctly tunes the radio to `415`, then pushed it to production in commit `e62dc3a`. The moment turns the puzzle solution into a memorable visual event before the signal key appears.
- **Files changed:** `script.js`, `PROJECT_MEMORY.md`
- **Technical Architecture changes or key technical decisions made:** Added `RIFT_DURATION`, `state.riftEffect`, `startRiftEffect()`, `updateRiftEffect()`, and Canvas render helpers for the rift tear, vines, spores, static bands, screen tint, flash, and `SIGNAL FOUND` text. Changed the radio success flow so the puzzle starts the rift first and only marks `libraryPuzzle.solved = true` after the effect finishes. Temporarily boosts the Demogorgon sound volume during the rift and restores it afterward.
- **Assumptions:** A 3-second event is long enough to feel cinematic without making the player wait too long. The rift should be a Canvas overlay rather than DOM UI because it belongs to the gameplay moment and can reuse existing game rendering.
- **Known limitations:** The rift has not been visually regression-tested with screenshots. The effect is code-drawn rather than asset-driven, so future art polish may require refactoring into reusable effect primitives or sprites.
